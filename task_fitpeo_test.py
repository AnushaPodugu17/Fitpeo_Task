...
def test_fitpeo():
    ...
    #Step-5: Updating the text feild to 520
    input_value.click() #clicking the text feild
    driver.execute_script("arguments[0].value = '';", input_value) #clearing the value
    input_value.send_keys(560)#sending the value 560
    time.sleep(3)

    #Step-6: Validating Slider Value
    #validating text
    Text_feild_value=input_value.get_attribute('value')
    assert Text_feild_value == '560', "Validated! The value is 560."
    #validating slider
    slider_point_pixel= int(slider_point.get_attribute('style').split("left: ")[1].split("%")[0])
    Expected_pixel_point=int((560*100)/int((slider_element.get_attribute('max'))))

    assert slider_point_pixel == Expected_pixel_point, "Slider has updated to 560"
    print('Slider has updated to 560')

    #Step-7: Selecting CPT codes
    ...
    assert a=='$75600' ,"Validated!"
    print("Validated Total Recurring Reimbursement for all Patients Per Month: $75600 ")
